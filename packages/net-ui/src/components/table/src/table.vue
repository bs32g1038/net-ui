<template>
    <div
        class="net-table"
        :class="{
            'net-table--fixHeight': this.height,
            'net-table--stripe': this.stripe,
            [`net-table--${this.size}`]: this.size,
            [`net-table--border`]: this.border,
        }"
        :style="tableStyles"
    >
        <!-- S Content -->
        <div class="net-table__content" :style="contentStyle">
            <!-- S Header -->
            <div class="net-table__header" v-if="height">
                <table>
                    <colgroup>
                        <col v-for="(column, index) in columnsData" :width="setCellWidth(column, index)" />
                    </colgroup>
                    <thead class="net-table__thead" ref="header">
                        <tr>
                            <!-- S Checkbox -->
                            <th v-if="optional" class="net-table__cell net-table__column-selection">
                                <net-checkbox v-model="isSelectAll" @click.native="handleSelectAll"></net-checkbox>
                            </th>
                            <!-- E Checkbox -->
                            <!-- S Column th -->
                            <th
                                v-for="(column, index) in columnsData"
                                class="net-table__cell net-table__column"
                            ...
                            >
                                ...
                                <template v-if="column.sortType">
                                    <div
                                        class="net-table__column-sorter"
                                        :class="{
                                            'sort-asc': column._sortType === 'asc',
                                            'sort-desc': column._sortType === 'desc',
                                        }"
                                    >
                                        <span class="net-table__column-sorter-up" @click.stop="handleSort(index, 'asc')"
                                            ><i class="icon icon-chevron-up"></i
                                        ></span>
                                        <span
                                            class="net-table__column-sorter-down"
            ...
            <!-- S Body -->
            <div class="net-table__body" :style="bodyStyle">
                <table>
                    <colgroup>
                        <col v-for="(column, index) in columnsData" :width="setCellWidth(column, index)" />
                    </colgroup>
                    <thead class="net-table__thead" v-if="!height" ref="header">
                        <tr>
                            <!-- S Checkbox -->
                            <th v-if="optional" class="net-table__cell net-table__column-selection">
                                <net-checkbox
                    ...
                    <tbody class="net-table__tbody" v-if="sortData.length" ref="body">
                        <template v-for="(item, index) in sortData">
                            <tr>
                                <td v-if="optional" class="net-table__cell net-table__column-selection">
                                    <net-checkbox
                                        v-model="objData[index].isChecked"
                                        @on-change="changeRowSelection"
                                    ></net-checkbox>
                                </td>
                                <td v-for="(column, cindex) in columns" class="net-table__cell">
                    ...
                    <tbody class="net-table__tbody" v-else>
                        <tr>
                            <td
                                class="net-table__cell net-table__cell--nodata"
        ...
        <!-- S Pagination -->
        <div v-if="pagination && total" class="net-table__footer" ref="footer">
            <net-pagination
                :current="currentPage"
                :size="size"
                :total="total"
...
import { getStyle, deepCopy } from 'net-ui/src/utils/util';
import Cell from './render';
import Checkbox from 'net-ui/src/components/checkbox';
...
export default {
    ...
    methods: {
        ...
        getSelection() {
            ...
            return JSON.parse(
                JSON.stringify(this.data.filter((data, index) => selectionIndexArray.indexOf(index) > -1))
            );
        },
        changeRowSelection() {
            const selection = this.getSelection();
