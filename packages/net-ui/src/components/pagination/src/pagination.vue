<template>
    ...
    <ul
        v-if="simple"
        class="net-pagination net-pagination--simple"
        :class="{
            [`net-pagination--${size}`]: size,
        }"
    >
        <li
            :title="t('at.pagination.prevText')"
            class="net-pagination__prev"
            :class="{
                'net-pagination--disabled': this.currentPage === 1,
            }"
            @click="handlePrev"
        >
            <i class="icon icon-chevron-left"></i>
        </li>
        <div class="net-pagination__simple-paging">
            <input
                type="text"
            ...
            />
            <span>/</span>
            <span class="net-pagination__paging-total">{{ totalPage }}</span>
        </div>
        <li
            :title="t('at.pagination.nextText')"
            class="net-pagination__next"
            :class="{
                'net-pagination--disabled': this.currentPage === this.totalPage,
            }"
            @click="handleNext"
        >
    ...
    <ul
        v-else
        class="net-pagination"
        :class="{
            [`net-pagination--${size}`]: size,
        }"
    >
        <span class="net-pagination__total" v-show="showTotal">
        ...
        <template v-if="totalPage < 9">
            <li
                ...
                class="net-pagination__item"
                :class="{
                    'net-pagination__item--active': currentPage === num,
                }"
        ...
        <template v-else>
            <li
                class="net-pagination__item"
                :class="{ 'net-pagination__item--active': currentPage === 1 }"
                @click="changePage(1)"
            >
                1
            </li>
            <li
                class="net-pagination__item net-pagination__item--jump-prev"
            ...
            <li class="net-pagination__item" v-if="currentPage > 3" @click="changePage(currentPage - 2)">
                {{ currentPage - 2 }}
            </li>
            <li class="net-pagination__item" v-if="currentPage > 2" @click="changePage(currentPage - 1)">
                {{ currentPage - 1 }}
            </li>
            <li
                class="net-pagination__item net-pagination__item--active"
                v-if="currentPage !== 1 && currentPage !== totalPage"
            >
                {{ currentPage }}
            </li>
            <li class="net-pagination__item" v-if="currentPage < totalPage - 1" @click="changePage(currentPage + 1)">
                {{ currentPage + 1 }}
            </li>
            <li class="net-pagination__item" v-if="currentPage < totalPage - 2" @click="changePage(currentPage + 2)">
                {{ currentPage + 2 }}
            </li>
            <li
                class="net-pagination__item net-pagination__item--jump-next"
            ...
            <li
                class="net-pagination__item"
                v-if="totalPage > 1"
                :class="{ 'net-pagination__item--active': currentPage === totalPage }"
        ...
        <div v-if="showSizer" class="net-pagination__sizer">
            <net-select v-model="currentPageSize" :size="size" @on-change="changeSize">
                <net-option v-for="item in pageSizeOpts" :key="item" :value="item">{{
                    `${item} ${t('at.pagination.pageSize')}`
                }}</net-option>
            </net-select>
        </div>
        <div class="net-pagination__quickjump" v-if="showQuickjump">
