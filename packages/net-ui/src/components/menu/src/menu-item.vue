<template>
    <li
        class="net-menu__item"
        :class="[this.active ? 'net-menu__item--active' : '', this.disabled ? 'net-menu__item--disabled' : '']"
        @click="handleClick"
    >
        <router-link v-if="Object.keys(to).length" class="net-menu__item-link" ref="link" :to="to">
            <slot></slot>
        </router-link>
        <div v-else class="net-menu__item-link">
            <slot></slot>
        </div>
    </li>
</template>
