<template>
    <div
        :class="[
            'net-select',
            {
                'net-select--visible': this.visible,
                'net-select--disabled': this.disabled,
                'net-select--multiple': this.multiple,
                'net-select--single': !this.multiple,
                'net-select--show-clear': this.showCloseIcon,
                [`net-select--${this.size}`]: !!this.size,
            },
        ]"
        v-clickoutside="handleClose"
    >
        <!-- S Selection -->
        <div class="net-select__selection" ref="trigger" @click="toggleMenu">
            <span class="net-tag" v-for="(item, index) in selectedMultiple">
                <span class="net-tag__text">{{ item.label }}</span>
                <i class="icon icon-x net-tag__close" @click.stop="removeTag(index)"></i>
            </span>
            <span class="net-select__placeholder" v-show="showPlaceholder && !filterable">{{ localePlaceholder }}</span>
            <span class="net-select__selected" v-show="!showPlaceholder && !multiple && !filterable">{{
                selectedSingle
            }}</span>
            <input
                type="text"
                class="net-select__input"
                ...
                ref="input"
            />
            <i class="icon icon-chevron-down net-select__arrow"></i>
            <i class="icon icon-x net-select__clear" v-show="showCloseIcon" @click.stop="clearSingleSelect"></i>
        </div>
        <!-- E Selection -->
        ...
        <transition name="slide-up" @after-leave="doDestory">
            <div
                class="net-select__dropdown"
                :class="[placement ? `net-select__dropdown--${placement}` : 'net-select__dropdown--bottom']"
                v-show="visible"
                ref="popover"
            >
                <ul v-show="notFound" class="net-select__not-found">
                    <li>{{ localeNotFoundText }}</li>
                </ul>
                <ul v-show="!notFound" class="net-select__list" ref="options">
                    <slot></slot>
                </ul>
            </div>
        </transition>
            ...
            options.forEach(option => {
                ...
                if (index > -1) {
                    ...
                    valueLabelArr[index].label =
                        typeof option.label === 'undefined' ? option.$el.innerHTML : option.label;
                } else {
                    option.selected = false;
                }
            });
