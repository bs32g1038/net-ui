<template>
    <div
        class="net-tabs"
        :class="{
            'net-tabs--small': size === 'small',
            'net-tabs--card': type === 'card',
            'net-tabs--scroll': scrollable,
        }"
    >
        <div class="net-tabs__header">
            <div class="net-tabs__extra" v-if="$slots.extra">
                <slot name="extra"></slot>
            </div>
            <div class="net-tabs__nav">
                <!-- S prev btn -->
                <span
                    class="net-tabs__prev"
                    v-if="scrollable"
                    :class="{
                        'net-tabs__prev--disabled': !prevable,
                    }"
                    @click="scrollPrev"
                >
                ...
                <!-- S next btn -->
                <span
                    class="net-tabs__next"
                    v-if="scrollable"
                    :class="{
                        'net-tabs__next--disabled': !nextable,
                    }"
                    @click="scrollNext"
                >
                ...
                <!-- E next btn -->
                <!-- S Tab nav -->
                <div class="net-tabs__nav-wrap">
                    <div class="net-tabs__nav-scroll" ref="navScroll">
                        <div class="net-tabs-nav" ref="nav" :style="navStyle">
                            <div
                                class="net-tabs-nav__item"
                                :class="{
                                    'net-tabs-nav__item--active': index === activeIndex,
                                    'net-tabs-nav__item--disabled': item.disabled,
                                    'net-tabs-nav__item--closable': item.closable,
                                }"
        ...
        <div class="net-tabs__body" :style="tabsBodyTranslateStyle">
            <slot></slot>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        scrollToActiveTab() {
            ...
            const activeTab = this.$el.querySelector('.net-tabs-nav__item--active');
        ...
        removeHandle(index) {
            ...
            if (tab.currentName === this.activeKey) {
                ...
                if (newTabs.length) {
                    const nextAbleTabs = tabs.filter((item, itemIndex) => !item.disabled && itemIndex > index);

                    const prevAbleTabs = tabs.filter((item, itemIndex) => !item.disabled && itemIndex < index);
